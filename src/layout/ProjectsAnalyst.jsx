    ...
    return (
        <>
            <div className="flex flex-col items-center mt-20" id="portfolio" >
                <h2 className="text-2xl desktop:text-3xl font-semibold font-title">Proyectos en Análisis de Datos</h2>

                <p className="w-4/5 text-sm m-4 large-desktop:text-base font-title text-center desktop:w-2/4  large-desktop:w-1/3">Te invito a echar un vistazo a mis Dashboards y informes con Power Bi. Cada proyecto que encontrarás aquí es una pieza clave en mi aprendizaje y evolución en el Análisis de Datos.</p>
            </div>

            {
                projectsAnalyst.map(({ name, id, img, description, github, deploy }) => (
                    <div className="mb-11 flex justify-center text-white flex-col desktop:flex-row items-center" key={id} >
                        ...
                        <div className="flex items-center w-4/5 h-48 rounded-xl bg-customProject desktop:w-4/12 desktop:h-72 large-desktop:h-80 large-desktop:w-1/4">
                            <img className="imageProject large-desktop:w-[400px] w-[300px] mx-auto" src={img} alt={name} />
                        </div>

                        <div className="flex flex-col items-center w-4/5 justify-center bg-dark rounded-xl desktop:h-72 desktop:w-4/12 large-desktop:h-80 large-desktop:w-1/4">

                            <h3 className="mt-3 desktop:mb-3 font-title desktop:text-2xl">{name}</h3>
