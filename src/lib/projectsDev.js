export const projectsDev = [
  {
    id: 1,
    name: "Oliva's Deli",
    description:
      "Sitio web para emprendimiento de comidas artesanales, diseñado para mostrar productos, servicios de catering y postres. las consultas y pedidos se realizan de forma directa a través de WhatsApp, facilitando una comunicación rápida y personalizada con los clientes.",
    img: "./img/projects/olivasdelii.png",
    github: "https://github.com/MaikolReyes/Olivas-Deli",
    deploy: "https://olivasdeli.com.ar/",
  },
  {
    id: 2,
    name: "Set Tenis",
    description:
      "Sitio ecommerce para tienda de remeras de tenis y pádel, diseñado para exhibir los productos de forma clara y ordenada. La tienda permite realizar compras de manera simple y rápida, ofreciendo una experiencia práctica para el usuario.",
    img: "./img/projects/settenis.png",
    github: "https://github.com/MaikolReyes",
    deploy: "https://tiendasettenis.com.ar/",
  },
  {
    id: 2,
    name: "Zulia Marketing Digital",
    description:
      "Es un sitio web para una agencia de marketing digital que ofrece servicios de desarrollo web y publicidad en RRSS. Esta realizada con React, TypeScript, SASS y Boostraps para los estilos.",
    img: "./img/projects/zmd.png",
    github: "https://github.com/MaikolReyes/Zulia-Marketing-Digital",
    deploy: "https://zulia-marketing-digital.onrender.com/",
  },
  {
    id: 3,
    name: "Franquicias360",
    description:
      "Es un sitio web para una agencia de marketing de franquicias en el que utilice como lenguaje de programacion React, Javascript y para los estilos de la página utilice el preprocesador SASS.",
    img: "./img/projects/franquicias.png",
    github: "https://github.com/MaikolReyes/Franquicias360",
    deploy: "https://franquicias360.netlify.app/",
  },
];
